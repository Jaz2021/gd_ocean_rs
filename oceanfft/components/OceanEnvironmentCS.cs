// @icon("res://addons/oceanfft/icons/OceanEnvironment.svg")
using System;
...
public partial class OceanEnvironmentCS : Node3D
{
    ...
    [Export] Ocean ocean;
    [Export] private bool ShowInEditor{
        get {
            return IsProcessing();
        }
        set
    ...
    [Export(PropertyHint.Range, "1,120,1")] private float OceanFramerate {
        get {
            return oceanFramerate;
        }
        set {
            // Console.Log($"Ocean framerate changed to {value}, so OceanDelta is now {1f / value}", Console.MessageType.Error);
            if(float.IsInfinity(value)){
                return;
            }
            oceanFramerate = value;
            if(oceanFramerate != 0){
                oceanDelta = 1f / value;
                if(ocean.initialized){
                    ocean.material.SetShaderParameter("sim_time", 1f / value);
                }
            }
        }
    }
    private float oceanFramerate = 60f;
    private float oceanDelta = 1f / 60f;
    ...
    public override void _EnterTree()
    {
        ...
        camera = editor.GetEditorViewport3D().GetCamera3D();
        if(camera == null){
            GD.Print("Failed to find camera");
        }
        // Globals.instance.oceanEnvironment = this;
        // if (ocean?.initialized == false)
        // {
    ...
    public override void _Ready()
    {
        if(Engine.IsEditorHint()){
            ...
            var editor = EditorInterface.Singleton;
            camera = editor.GetEditorViewport3D().GetCamera3D();
            return; // Ignore ready in editor. Use EnterTree instead
        } else {
            SetProcess(true);
        }
        ...
        oceanDelta = 1f / oceanFramerate;
        
    }
    public override void _Process(double delta)
    {
        // Setting up flat Ocean framerate:
        // var camera = GetViewport().GetCamera3D();
        curDelta += (float)delta;
        if(curDelta >= oceanDelta){
            OceanFrame(curDelta);
            curDelta = 0;
        } else {
            if(ocean == null){
            ...
            if(camera == null){
                ...
                camera = Engine.IsEditorHint() ? EditorInterface.Singleton.GetEditorViewport3D().GetCamera3D() : GetViewport().GetCamera3D();
                return;
            }
            if (!ocean.initialized)
            {
                ocean.InitializeSimulation(oceanDelta);
            }
            ocean.EstimateSimulate((float)delta);
        }

    }
    private void OceanFrame(float delta){
        if(ocean == null){
            GD.Print("Ocean null");
            return;
        }
    ...
    public float GetWaveHeight(Vector3 global_pos, int max_cascade = 1, int steps = 2)
    {
        ...
        return ocean.GetWaveHeight(camera, global_pos, max_cascade, steps);
    }
